...
func TestCuckooFilterAddTest(t *testing.T) {
	f := NewCuckooFilter()
...
func TestCuckooFilterMultipleHashes(t *testing.T) {
	f := NewCuckooFilter()
...
func TestCuckooFilterFalsePositiveRate(t *testing.T) {
	...
	f := NewCuckooFilterWithCapacity(10000)
...
func BenchmarkCuckooFilterAddAndTest(b *testing.B) {
	f := NewCuckooFilter()
	b.ResetTimer()
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
