...
Memory usage is near zero regardless of duplicate ratio, while maintaining faster operations.
...
func main() {
    ...
    // These duplicate logs will be suppressed within window
...
handler := slogdedupe.NewHandler(
    ...
    slogdedupe.WithOnDrop(func(ctx context.Context, r slog.Record) {
        // callback when duplicate is dropped
    }),
)
...
Set which log properties determine uniqueness
...
// Cuckoo filter - not performant, mostly just for testing
...
The bloom filter provides probabilistic deduplication with a configurable false positive rate. This means a small percentage of unique logs may be incorrectly identified as duplicates, but memory usage remains constant and minimal regardless of log volume.
